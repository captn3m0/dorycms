...
require_once("class.postfilter.php");
...
class dbObject
{
	...
	*/
	...
	public static function importArray($className, $input) 
	{
		...
		return(sizeof($output) > 0 ? $output : array());	
	}
...
class QueryBuilder 
{
	...
	private function buildFilters($what, $value, $class)
	{
		...
		elseif(is_numeric($what)) { // it's a custom whereclause (not just $field=>$value)		
			...
			//AEON_CHANGE: $value = dbConnection::getInstance($this->class->databaseInfo->connection)->escapeValue($value);
